...
static void light_sensors_update_and_maybe_request_turn(uint16_t* V4_pp, uint16_t* V5_pp, uint16_t* V6_pp)
{
    ...
    uint8_t s1_exit  = (V1 > (S_MAXC_COUNTS + S_HYST_COUNTS)) || ((V1 + S_HYST_COUNTS) < S_MINC_COUNTS);
    uint8_t s2_exit  = (V2 > (S_MAXC_COUNTS + S_HYST_COUNTS)) || ((V2 + S_HYST_COUNTS) < S_MINC_COUNTS);

    if (!s1) s1 = s1_enter ? 1u : 0u; else s1 = s1_exit ? 0u : 1u;
    if (!s2) s2 = s2_enter ? 1u : 0u; else s2 = s2_exit ? 0u : 1u;

    sen1_on_line = s1;  sen2_on_line = s2;
    ...
    static uint8_t debL = 0, debR = 0, latched_any = 0, armed = 0, clear_cnt = 0;

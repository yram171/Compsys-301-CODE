...
void isr_qd_Stop(void)
{
    ...
    isr_qd_SetVector(&IntDefaultHandler);
}
...
{
    #ifdef isr_qd_INTERRUPT_INTERRUPT_CALLBACK
        isr_qd_Interrupt_InterruptCallback();
    #endif /* isr_qd_INTERRUPT_INTERRUPT_CALLBACK */ 
...
void isr_qd_SetVector(cyisraddress address)
{
    ...
    ramVectorTable[CYINT_IRQ_BASE + (uint32)isr_qd__INTC_NUMBER] = address;
}
...
cyisraddress isr_qd_GetVector(void)
{
    ...
    return ramVectorTable[CYINT_IRQ_BASE + (uint32)isr_qd__INTC_NUMBER];
}
...
*
*   Note calling isr_qd_Start or isr_qd_StartEx will 
*   override any effect this API would have had. This API should only be called
*   after isr_qd_Start or isr_qd_StartEx has been called. 
...
uint8 isr_qd_GetState(void)
{
    ...
    return ((*isr_qd_INTC_SET_EN & (uint32)isr_qd__INTC_MASK) != 0u) ? 1u:0u;
}

...
static volatile int32_t TICKS_90_LEFT  = 880;   /* abs(|ΔL|)+abs(|ΔR|) */
static volatile int32_t TICKS_90_RIGHT = 900;

/* Pivot speed (%) — keep modest to avoid overshoot */
#define PIVOT_SPEED_PC        28
#define STOP_BEFORE_MS        80
#define BRAKE_AFTER_MS        90

/* Safety: max number of handler calls allowed while turning.
 * With your ~8 ms loop this is ~3.2 s (400 * 8 ms) which is plenty. */
#define MAX_TURN_HANDLER_TICKS  400
 ...
 */
static void pivot_left_speed(int pct)
{
    int L = -pct;   // reverse
    int R = +pct;   // forward
    int base  = (L + R) / 2;
...
static void pivot_right_speed(int pct)
{
    int L = +pct;
    int R = -pct;
    int base  = (L + R) / 2;
...
void Directions_Handle(volatile uint8_t* p_dir)
{
    ...
    case DIR_TURNING:
        /* Drive the pivot */
        if (s_turn_side == 1u) {
            pivot_left_speed(PIVOT_SPEED_PC);
        } else {
            pivot_right_speed(PIVOT_SPEED_PC);
        }
